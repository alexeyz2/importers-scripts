...
rem set PACKAGING_BASE_URI=s3://%S3BUCKET%
set ALINE_METAINF_JSON_FILE=%BASE_DIR%\logs\metainf\metainf.json
set MODULES_KEY=modules/

rem WARNING THESE VARIABLES SHOULD COME ON MDM, I'M JUST SETTING HERE FOR TESTING PURPOSES
rem S3 | MINIO | LOCAL
set STORAGE_PROVIDER="MINIO"
rem THIS IS THE NAME OF THE BUCKET, eg s3 -> s3://aline-build-output-p9  | minio -> http://127.0.0.1:9000/minio/aline-build-output-p9
rem IN EITHER CASE THIS VARIABLE SHOULD BE SET AS "aline-build-output-p9". CURRENTLY MDM GIVE US $AWS_S3_BUCKET, THIS SHOULD BE
rem CHANGED TO STORAGE_BUCKET TO WORK FOR BOTH S3 AND MINIO
set STORAGE_BUCKET=$S3BUCKET
rem THIS VARIABLE ONLY NEEDS TO BE SET WHEN STORAGE_PROVIDER=MINIO , IN CASE OF S3 THIS CAN REMAIN EMPTY!
rem THIS SHOULD COME AS MDM PARAMETER, IT'S HARD CODED TO TEST ON LOCALHOST, BUT SHOULD BE CHANGED TO ANOTHER IP IN CASE MINIO IS NOT RUNNING LOCALLY
set STORAGE_HOST="http://127.0.0.1:9000"
rem MINIO GENERATED KEYS, SHOULD COME AS MDM PARAMETER, HARD CODED FOR TESTING PURPOSES, SET IT ACCORDING TO YOUR ENVIRONMENT
set STORAGE_ACCESS_KEY_ID="8P4XJ82BN1LPTMUCVR1Y"
set STORAGE_SECRET_ACCESS_KEY="M31UrmDqoRjHq6PHumHcaILGNgykU/hi3GgfOZbD"
